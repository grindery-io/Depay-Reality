...
describe("Grindery Pool testings", function () {
  ...
  beforeEach(async function() {
    ...
    grtPool = await upgrades.deployProxy(await ethers.getContractFactory("contracts/v0.1.0/GrtPool.sol:GrtPool"));
    await grtPool.deployed();

    grtSatellite = await upgrades.deployProxy(await ethers.getContractFactory("contracts/v0.1.0/GrtSatellite.sol:GrtSatellite"));
  ...
  describe("Deposit GRT and request ERC20 tokens", function () {
    ...
    it("Should fail if the deposit has already been submitted", async function () {
      await grtPool.connect(user1).depositGRTRequestERC20(0, 10, token.address, 1000, chainId, user1.address);
      await expect(
				grtPool.connect(user1).depositGRTRequestERC20(0, 10, token.address, 1000, chainId, user1.address)
    ...
    it("Should emit a new deposit event", async function () {
      const encodePacked =  ethers.utils.solidityPack(
        ["uint256", "address", "address", "uint256", "uint256", "address", "uint256", "uint256", "address" ],
        [0, user1.address, grtToken.address, 10, chainId, token.address, 1000, chainId, user1.address]
      );
      await expect(
        await grtPool.connect(user1).depositGRTRequestERC20(0, 10, token.address, 1000, chainId, user1.address)
      )
			.to.emit(grtPool, "LogDeposit")
			.withArgs(ethers.utils.keccak256(encodePacked) , grtToken.address, 10, chainId);
    });
    ...
    it("Should emit a new request event", async function () {
      ...
      await expect(
        await grtPool.connect(user1).depositGRTRequestERC20(0, 10, token.address, 1000, chainId, user1.address)
			...
			.withArgs(ethers.utils.keccak256(encodePacked), token.address, 1000, chainId);
    });
    ...
    describe("General information", function () {
      ...
      it("Should add a new item in the requests mapping with the proper requester", async function () {
        for (let i = 0; i < nbrRequest; i++) {
          const encodePacked =  ethers.utils.solidityPack(
            ["uint256", "address", "address", "uint256", "uint256", "address", "uint256", "uint256", "address" ],
            [i, user1.address, grtToken.address, 10, chainId, token.address, 1000, chainId, user1.address]
          );
          await grtPool.connect(user1).depositGRTRequestERC20(i, 10, token.address, 1000, chainId, user1.address);
          expect(await grtPool.getRequester(ethers.utils.keccak256(encodePacked))).to.equal(user1.address);
        }
      });
      ...
      it("Should add a new item in the requests mapping with the proper recipient address", async function () {
        for (let i = 0; i < nbrRequest; i++) {
          const encodePacked =  ethers.utils.solidityPack(
            ["uint256", "address", "address", "uint256", "uint256", "address", "uint256", "uint256", "address" ],
            [i, user1.address, grtToken.address, 10, chainId, token.address, 1000, chainId, user2.address]
          );
          await grtPool.connect(user1).depositGRTRequestERC20(i, 10, token.address, 1000, chainId, user2.address);
          expect(await grtPool.getRecipient(ethers.utils.keccak256(encodePacked))).to.equal(user2.address);
        }
      });
      ...
      it("Should set isRequest to true in the request mapping", async function () {
        for (let i = 0; i < nbrRequest; i++) {
          ...
          await grtPool.connect(user1).depositGRTRequestERC20(i, 10, token.address, 1000, chainId, user2.address);
          expect(await grtPool.isrequest(ethers.utils.keccak256(encodePacked))).to.equal(true);
        }
      });
    ...
    describe("Deposit information", function () {
      ...
      it("Should provide the correct deposit token address (in the requests mapping)", async function () {
        for (let i = 0; i < nbrRequest; i++) {
          ...
          await grtPool.connect(user1).depositGRTRequestERC20(i, 10, token.address, 1000, chainId, user2.address);
          expect(await grtPool.getDepositToken(ethers.utils.keccak256(encodePacked))).to.equal(grtToken.address);
        }
      });
      ...
      it("Should provide the correct deposit token amount (in the requests mapping)", async function () {
        for (let i = 0; i < nbrRequest; i++) {
          ...
          await grtPool.connect(user1).depositGRTRequestERC20(i, 10, token.address, 1000, chainId, user2.address);
          expect(await grtPool.getDepositAmount(ethers.utils.keccak256(encodePacked))).to.equal(10);
        }
      });
      ...
      it("Should provide the correct deposit chain Id (in the requests mapping)", async function () {
        for (let i = 0; i < nbrRequest; i++) {
          ...
          await grtPool.connect(user1).depositGRTRequestERC20(i, 10, token.address, 1000, chainId, user2.address);
         expect(await grtPool.getDepositChainId(ethers.utils.keccak256(encodePacked))).to.equal(chainId);
        }
      });
    ...
    describe("Request information", function () {
      ...
      it("Should provide the correct request token address (in the requests mapping)", async function () {
        for (let i = 0; i < nbrRequest; i++) {
          ...
          await grtPool.connect(user1).depositGRTRequestERC20(i, 10, token.address, 1000, chainId, user2.address);
          expect(await grtPool.getRequestToken(ethers.utils.keccak256(encodePacked))).to.equal(token.address);
        }
      });
      ...
      it("Should provide the correct request token amount (in the requests mapping)", async function () {
        for (let i = 0; i < nbrRequest; i++) {
          ...
          await grtPool.connect(user1).depositGRTRequestERC20(i, 10, token.address, 1000, chainId, user2.address);
          expect(await grtPool.getRequestAmount(ethers.utils.keccak256(encodePacked))).to.equal(1000);
        }
      });
      ...
      it("Should provide the correct request chain Id (in the requests mapping)", async function () {
        for (let i = 0; i < nbrRequest; i++) {
          ...
          await grtPool.connect(user1).depositGRTRequestERC20(i, 10, token.address, 1000, chainId, user2.address);
          expect(await grtPool.getRequestChainId(ethers.utils.keccak256(encodePacked))).to.equal(chainId);
        }
      });
  ...
  describe("Deposit GRT and request native tokens", function () {
    ...
    it("Should fail if the deposit has already been submitted", async function () {
      ...
      await grtPool.connect(user1).depositGRTRequestNative(
        0,
        10,
        1000,
        chainId,
    ...
    it("Should emit a new deposit event", async function () {
      const encodePacked =  ethers.utils.solidityPack(
        ["uint256", "address", "address", "uint256", "uint256", "address", "uint256", "uint256", "address" ],
        [0, user1.address, grtToken.address, 10, chainId, ethers.constants.AddressZero, 1000, chainId, user1.address]
      );
      await expect(
        await grtPool.connect(user1).depositGRTRequestNative(0, 10, 1000, chainId, user1.address)
      )
			.to.emit(grtPool, "LogDeposit")
			.withArgs(ethers.utils.keccak256(encodePacked), grtToken.address, 10, chainId);
    });
    ...
    it("Should emit a new request event", async function () {
      ...
      await expect(
        await grtPool.connect(user1).depositGRTRequestNative(0, 10, 1000, chainId, user1.address)
			...
			.to.emit(grtPool, "LogRequest")
			.withArgs(ethers.utils.keccak256(encodePacked), ethers.constants.AddressZero, 1000, chainId);
    });
    ...
    describe("General information", function () {
      ...
      it("Should add a new item in the requests mapping with the proper requester", async function () {
        for (let i = 0; i < nbrRequest; i++) {
          const encodePacked =  ethers.utils.solidityPack(
            ["uint256", "address", "address", "uint256", "uint256", "address", "uint256", "uint256", "address" ],
            [i, user1.address, grtToken.address, 10, chainId, ethers.constants.AddressZero, 1000, chainId, user1.address]
          );
          await grtPool.connect(user1).depositGRTRequestNative(i, 10, 1000, chainId, user1.address);
          expect(await grtPool.getRequester(ethers.utils.keccak256(encodePacked))).to.equal(user1.address);
        }
      });
      ...
      it("Should add a new item in the requests mapping with the proper recipient address", async function () {
        for (let i = 0; i < nbrRequest; i++) {
          const encodePacked =  ethers.utils.solidityPack(
            ["uint256", "address", "address", "uint256", "uint256", "address", "uint256", "uint256", "address" ],
            [i, user1.address, grtToken.address, 10, chainId, ethers.constants.AddressZero, 1000, chainId, user2.address]
          );
          await grtPool.connect(user1).depositGRTRequestNative(i, 10, 1000, chainId, user2.address);
          expect(await grtPool.getRecipient(ethers.utils.keccak256(encodePacked))).to.equal(user2.address);
        }
      });
    ...
    describe("Request information", function () {
      ...
      it("Should provide the correct request token address (zero address in the requests mapping)", async function () {
        for (let i = 0; i < nbrRequest; i++) {
          ...
          await grtPool.connect(user1).depositGRTRequestNative(i, 10, 1000, chainId, user2.address);
          expect(await grtPool.getRequestToken(ethers.utils.keccak256(encodePacked))).to.equal(ethers.constants.AddressZero);
        }
      });
  ...
  describe("Create an offer", function () {
    ...
    beforeEach(async function() {
      await grtToken.connect(user1).mint(user1.address, 10000);
      await grtToken.connect(user1).approve(grtPool.address, 500);
      await grtPool.connect(user1).depositGRTRequestNative(0, 10, 1000, chainId, user2.address);

      const encodePacked =  ethers.utils.solidityPack(
        ["uint256", "address", "address", "uint256", "uint256", "address", "uint256", "uint256", "address" ],
        [0, user1.address, grtToken.address, 10, chainId, ethers.constants.AddressZero, 1000, chainId, user2.address]
      );
    ...
    it("Should fail if there is no request for the provided request Id", async function () {
      ...
      const encodePacked =  ethers.utils.solidityPack(
        ["uint256", "address", "address", "uint256", "uint256", "address", "uint256", "uint256", "address" ],
        [3, user1.address, grtToken.address, 10, chainId, ethers.constants.AddressZero, 1000, chainId, user2.address]
      );

      await expect(
				grtPool.connect(user1).createOffer(ethers.utils.keccak256(encodePacked), 1000)
    ...
    describe("Mapping details", function () {
      ...
      it("Should create a new offer with the correct creator", async function () {
        for (let i = 0; i < nbrOffer; i++) {
          await grtPool.connect(user2).createOffer(requestId, 1000);
          expect(await grtPool.getOfferCreator(requestId, i)).to.equal(user2.address);
        }
      });
  ...
  describe("Accept an offer", function () {
    ...
    it("Should fail if idRequest doesn't exist", async function () {
      const encodePacked =  ethers.utils.solidityPack(
        ["uint256", "address", "address", "uint256", "uint256", "address", "uint256", "uint256", "address" ],
        [1, user1.address, grtToken.address, 10, chainId, ethers.constants.AddressZero, 1000, chainId, user2.address]
      );
      await expect(
				grtPool.connect(user1).acceptOffer(ethers.utils.keccak256(encodePacked), 0)
			).to.be.revertedWith("GRT pool: the request does not exist!");
    });
  ...
  describe("Pay an offer on chain with an ERC20 token", function () {
    ...
    it("Should fail if the chain Id mentionned in the corresponding offer is not the actual chain Id", async function () {
      await grtPool.connect(user1).depositGRTRequestERC20(1, 10, token.address, 1000, 155, user2.address);
      const encodePacked =  ethers.utils.solidityPack(
        ["uint256", "address", "address", "uint256", "uint256", "address", "uint256", "uint256", "address" ],
        [1, user1.address, grtToken.address, 10, chainId, token.address, 1000, 155, user2.address]
      );
      await grtPool.connect(user2).createOffer(ethers.utils.keccak256(encodePacked), 1000);
      await grtPool.connect(user1).acceptOffer(ethers.utils.keccak256(encodePacked), 0);
      await expect(
        grtPool.connect(user2).payOfferOnChainERC20(ethers.utils.keccak256(encodePacked), 0)
    ...
    it("Should emit an event to declare the paid offer", async function () {
      await grtPool.connect(user1).acceptOffer(requestId, 0);
      await expect(await grtPool.connect(user2).payOfferOnChainERC20(requestId, 0))
  ...
  describe("Pay an offer on chain with native token", function () {
    ...
    beforeEach(async function() {
      ...
      await grtToken.connect(user1).mint(user1.address, 10000);
      await grtToken.connect(user1).approve(grtPool.address, 500);
      await grtPool.connect(user1).depositGRTRequestNative(0, 10, ethers.utils.parseEther("2"), chainId, user1.address);

      const encodePacked =  ethers.utils.solidityPack(
        ["uint256", "address", "address", "uint256", "uint256", "address", "uint256", "uint256", "address" ],
        [0, user1.address, grtToken.address, 10, chainId, ethers.constants.AddressZero, ethers.utils.parseEther("2"), chainId, user1.address]
      );
      ...
      await grtToken.connect(user2).approve(grtPool.address, 2000);
      await grtPool.connect(user2).stakeGRT(2);
      await grtPool.connect(user2).createOffer(requestId, ethers.utils.parseEther("2"));
    ...
    it("Should fail if the offer has already been paid", async function () {
      await grtPool.connect(user1).acceptOffer(requestId, 0);
      await grtPool.connect(user2).payOfferOnChainNative(requestId, 0, { value: ethers.utils.parseEther("2") });
      await expect(
        grtPool.connect(user2).payOfferOnChainNative(requestId, 0, { value: ethers.utils.parseEther("2") })
      ).to.be.revertedWith("GRT pool: the offer has already been paid!");
    });

    it("Should fail if the chain Id mentionned in the corresponding offer is not the actual chain Id", async function () {
      await grtPool.connect(user1).depositGRTRequestNative(1, 10, ethers.utils.parseEther("2"), 155, user2.address);
      const encodePacked =  ethers.utils.solidityPack(
        ["uint256", "address", "address", "uint256", "uint256", "address", "uint256", "uint256", "address" ],
        [1, user1.address, grtToken.address, 10, chainId, ethers.constants.AddressZero, ethers.utils.parseEther("2"), 155, user2.address]
      );

      await grtPool.connect(user2).createOffer(ethers.utils.keccak256(encodePacked), ethers.utils.parseEther("2"));
      await grtPool.connect(user1).acceptOffer(ethers.utils.keccak256(encodePacked), 0);
      await expect(
        grtPool.connect(user2).payOfferOnChainNative(ethers.utils.keccak256(encodePacked), 0)
      ).to.be.revertedWith("GRT pool: the offer should not be paid on this chain!");
    });
    ...
    it("Should fail if msg.value is not the promised amount", async function () {
      await grtPool.connect(user1).acceptOffer(requestId, 0);
      await expect(
        grtPool.connect(user2).payOfferOnChainNative(requestId, 0, { value: ethers.utils.parseEther("1") })
    ...
    it("Should fail if the transaction signer is not the one who made the offer", async function () {
      await grtPool.connect(user1).acceptOffer(requestId, 0);
      await expect(
        grtPool.connect(user3).payOfferOnChainNative(requestId, 0, { value: ethers.utils.parseEther("2") })
      ).to.be.revertedWith("GRT pool: you are not allowed to pay this offer!");
    });
    ...
    it("Should decrease the native token balance of the seller", async function () {
      ...
      const tx = await grtPool.connect(user2).payOfferOnChainNative(requestId, 0, { value: ethers.utils.parseEther("2") });
  ...
  describe("Claim GRT without dispute", function () {
    ...
    beforeEach(async function() {
      ...
      await grtPool.connect(user1).depositGRTRequestERC20(1, 10, token.address, 1000, chainId, user1.address);

      const encodePacked1 =  ethers.utils.solidityPack(
        ["uint256", "address", "address", "uint256", "uint256", "address", "uint256", "uint256", "address" ],
        [1, user1.address, grtToken.address, 10, chainId, token.address, 1000, chainId, user1.address]
      );
    ...
    describe("Native tokens", function () {
      ...
      it("Should fail if the request doesn't exist", async function () {
        ...
        const encodePacked =  ethers.utils.solidityPack(
          ["uint256", "address", "address", "uint256", "uint256", "address", "uint256", "uint256", "address" ],
          [10, user1.address, grtToken.address, 10, chainId, ethers.constants.AddressZero, ethers.utils.parseEther("2"), chainId, user1.address]
        );
        await expect(
          grtPool.connect(user2).claimGRTWithoutDispute(ethers.utils.keccak256(encodePacked), 0)
        ).to.be.revertedWith("GRT pool: the request does not exist!");
      });
      ...
      it("A successful GRT reward for the transaction signer should emit an event", async function () {
        await grtPool.connect(user1).acceptOffer(requestId, 0);
        grtSatellite.connect(user2).payOfferCrossChainNative(user1.address, { value: ethers.utils.parseEther("2") });
        await expect(await grtPool.connect(user2).claimGRTWithoutDispute(requestId, 0))
    ...
    describe("ERC20 tokens", function () {
      ...
      it("A successful GRT reward for the transaction signer should emit an event", async function () {
        await grtPool.connect(user1).acceptOffer(requestId1, 0);
        grtSatellite.connect(user2).payOfferCrossChainERC20(token.address, user1.address, 1000);
        await expect(await grtPool.connect(user2).claimGRTWithoutDispute(requestId1, 0))
        .to.emit(grtPool, "LogOfferPaidCrossChain")
