...
contract GrtPoolV2Tmp is OwnableUpgradeable, GrtOffer, UUPSUpgradeable {
    ...
    function depositETHAndAcceptOffer(
        ...
        return setTradeInfo(destAddr, address(0), msg.value, offerId, amountOffer);
    }
    ...
    function depositTestTokenAndAcceptOffer(
        ...
        require(token == _tokenTest, "Grindery Pool: the token sent must be the test token.");
    ...
    function setTradeInfo(
        ...
        bytes32 tradeId = keccak256(abi.encodePacked(msg.sender, _noncesDeposit[msg.sender], block.chainid));
    ...
    function setOrderComplete(bytes32 tradeId) external returns (bool) {
        ...
        require(msg.sender == _trades[tradeId].userAddr, "Grindery Pool: you are not the user who made the order.");
    ...
    function getPaymentInfo(
        bytes32 tradeId
    ) external view returns (bytes32 offerId, address destAddr, address token, uint256 amount) {
        return (
