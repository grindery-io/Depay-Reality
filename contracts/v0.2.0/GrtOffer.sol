...
contract GrtOffer is GrtTokenUtils, GrtOfferUtils {
    ...
    event LogSetPriceContractAddressOffer(bytes32 indexed _idOffer, address indexed _priceContractAddress);
    event LogSetLowerLimitOffer(bytes32 indexed _idOffer, bytes32 indexed _lowerLimitFn);
    event LogSetUpperLimitOffer(bytes32 indexed _idOffer, bytes32 indexed _upperLimitFn);
    ...
    function setChainIdOffer(bytes32 offerId, uint256 chainId) external {
        require(msg.sender == _offers[offerId].user, "you are not allowed to modify this offer");
    ...
    function setPriceContractAddressOffer(bytes32 offerId, address priceContractAddress) external {
        require(msg.sender == _offers[offerId].user, "you are not allowed to modify this offer");
    ...
    function setOffer(
        ...
        _offers[offerId].token = token;
        _offers[offerId].priceContractAddress = priceContractAddress;
        _offers[offerId].lowerLimitFn = keccak256(abi.encodePacked(lowerLimitFn));
        _offers[offerId].upperLimitFn = keccak256(abi.encodePacked(upperLimitFn));
        emit LogNewOffer(offerId, _offers[offerId].priceContractAddress, token, chainId);
    ...
    function checkParametersLowerLimitOffer(
        ...
        return
            keccak256(abi.encodePacked(args)) == _offers[offerId].lowerLimitFn &&
    ...
    function checkParametersUpperLimitOffer(
        ...
        return
            keccak256(abi.encodePacked(args)) == _offers[offerId].upperLimitFn &&
            priceContractAddress == _offers[offerId].priceContractAddress;
    }

    function getUpperPriceLimitFromContract(bytes32 offerId, bytes calldata fn) external view returns (uint256) {
        require(
        ...
        (bool success, bytes memory result) = _offers[offerId].priceContractAddress.staticcall(fn);
        if (success) {
    ...
    function getLowerPriceLimitFromContract(bytes32 offerId, bytes calldata fn) external view returns (uint256) {
        require(
            _offers[offerId].priceContractAddress != address(0),
            "GRT offers: not allowed to modify the price limit via an external smart contract"
        );
