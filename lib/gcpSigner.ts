...
const getSignerApply: ProxyHandler<any>['apply'] = function (
  ...
  if (signer) {
    return ethers.providers.Provider.isProvider(thisArg)
      ? signer.connect(thisArg)
      : signer;
  }
...
function patchField<T, N extends keyof T>(
  obj: T,
  prop: N,
  transformValue: (value: T[N], obj: T) => any
): T {
  let value = obj[prop];
  if (!value) {
...
class SignerWithAddressAlt extends ethers.Signer {
  ...
  public connect(provider: ethers.providers.Provider): SignerWithAddressAlt {
    return new SignerWithAddressAlt(
      this.address,
